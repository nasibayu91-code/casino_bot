...
"""ПОЛНОСТЬЮ ОБНОВЛЁННЫЕ ИГРЫ - ЧИСТЫЙ ЧАТ, ЗАЩИТА ОТ СПАМА, КРАСИВЫЙ UX"""
...
def is_locked(user_id):
    """Проверка блокировки от спама"""
    if user_id in user_locks:
        if time.time() - user_locks[user_id] < 3:  # 3 секунды блокировка
            return True
...
async def back_to_games(cb: CallbackQuery):
    """Вернуться в меню игр (РЕДАКТИРУЕМ сообщение, а не создаём новое)"""
...
async def dice_menu(cb: CallbackQuery):
    ...
    b.button(text="📈 БОЛЬШЕ 3", callback_data="dice_high")
    b.button(text="📉 МЕНЬШЕ 4", callback_data="dice_low")
    b.button(text="🔙 Назад", callback_data="menu_games")
    b.adjust(3, 2, 2, 1)
...
"""
    ...
    # Редактируем меню (не новое сообщение!)
    await cb.message.edit_text(text, reply_markup=b.as_markup(), parse_mode="HTML")
...
async def slots_play(cb: CallbackQuery):
    ...
    # Анимация (быстрая смена символов в одном сообщении)
...
async def crash_start(cb: CallbackQuery):
    ...
    # Сохраняем сессию
    if not hasattr(db, 'sessions'):
        db.sessions = {}
    db.sessions[str(cb.from_user.id)] = {"crash": crash_point, "mult": 1.0, "bet": 100}
...
async def crash_animation(msg, user_id):
    ...
    for _ in range(40):
        if not hasattr(db, 'sessions') or str(user_id) not in db.sessions:
            break
        
        session = db.sessions[str(user_id)]
...
async def crash_cashout(cb: CallbackQuery):
    ...
    if not hasattr(db, 'sessions') or str(cb.from_user.id) not in db.sessions:
        await cb.answer("❌ Игра завершена!", show_alert=True)
        return
    
    session = db.sessions[str(cb.from_user.id)]
    mult = session["mult"]
    ...
    del db.sessions[str(cb.from_user.id)]
...
async def mines_start(cb: CallbackQuery):
    ...
    if not hasattr(db, 'sessions'):
        db.sessions = {}
    db.sessions[str(cb.from_user.id)] = {"board": board, "opened": [], "bet": 150, "mult": 1.0}
...
async def mines_open(cb: CallbackQuery):
    ...
    s = db.sessions.get(str(cb.from_user.id))
